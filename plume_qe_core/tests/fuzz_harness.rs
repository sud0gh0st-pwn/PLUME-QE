use rand::Rng;
use plume_qe_core::{decrypt, encrypt, keygen_bundle, mix_seed_with_fingerprint, PolymorphismEngine};
...
fn fuzz_placeholder_roundtrip() {
    ...
    for idx in 0..16u8 {
        ...
        let ciphertext = encrypt(&engine, &bundle.public, &seed_bytes, idx as u64, &msg, &mut noise)
            .expect("encrypt");
        let recovered = decrypt(&engine, &bundle.secret, &seed_bytes, idx as u64, &ciphertext)
            .expect("decrypt");
